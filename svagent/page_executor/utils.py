...
def call_dino(instruction, screenshot_path):
    ...
    response = requests.post("http://127.0.0.1:24020/v1/executor", files=files,
                             data={"text_prompt": f"{instruction}"})
    return [int(s) for s in response.json()['response'].split(',')]



def get_center_width_height(relative_bbox, viewport_width, viewport_height):
    ...
    return center_x, center_y, width_x, height_y
