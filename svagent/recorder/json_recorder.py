...
class JSONRecorder:
    def __init__(self, id, instruction, page_executor, config):
        ...
        self.turn_number = 0
        trace_dir = os.path.join(config.get('task_dir', './task_output'), 'traces')
        xml_dir = os.path.join(config.get('task_dir', './task_output'), 'xml')
        log_dir = config.get('task_dir', './task_output')
        ...
        if config.get('version') is None or config.get('version') == "v1":
            self.xml_compressed_version = "v1"
        elif config.get('version') == "v2":
            self.xml_compressed_version = "v2"

    def check_validy_path(self, controller, need_screenshot=False, ac_status=False):
        if need_screenshot:
        ...
        return xml_path, ac_xml_path


    def update_response_deprecated(self, controller, response=None, prompt="** screenshot **", need_screenshot=False,
                                   ac_status=False):
        xml_path, ac_xml_path = self.check_validy_path(controller, need_screenshot, ac_status)
        step = {
            "trace_id": self.id,
